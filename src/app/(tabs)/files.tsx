...
export default function FilesScreen() {
      ...
      const fileDetails: LocalFile[] = await Promise.all(
        items.map(async (name: string) => {
          ...
          try {
            ...
            return { name, uri, size: 0, modificationTime: 0, isDirectory: false };
          }
        })
      );
  ...
  async function handleShare(file: LocalFile) {
    try {
      ...
      await Sharing.shareAsync(file.uri, { dialogTitle: `Share ${file.name}` });
    } catch (e) {
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptySubtitle}>Tap + Import to add files</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => item.isDirectory ? handleOpenFolder(item) : handleLongPress(item)}
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    gap: 8, borderBottomWidth: 1, borderBottomColor: '#2a2a2a',
  },
  headerTop:      { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  headerTitle:    { fontSize: 24, fontWeight: '800', color: '#FFFFFF' },
  headerSubtitle: { fontSize: 12, color: '#555555', marginTop: 2 },
  iconBtn:        { backgroundColor: '#f97316', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
  ...
  fileRow: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#1f1f1f', gap: 12,
  },
